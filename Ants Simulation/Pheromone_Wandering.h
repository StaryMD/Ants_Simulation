...
struct Pheromone_Wandering : Pheromone {
	...
	Pheromone_Wandering(sf::Vector2f _pos) : pos(_pos) {
		age = 0;
	}
	...
	void draw() {
		...
		Pheromone_Shape.setFillColor(sf::Color(0, 0, (uint8_t)255, (uint8_t)(opacity * 255.0f)));
		window.draw(Pheromone_Shape);
	}

	uint8_t getType() {
		return PHEROMONE_WANDERING_TYPE;
	}
