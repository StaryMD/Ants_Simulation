...
struct World_Map {
  ...
  void load_map_from_file(const char *fileName,
    ...
    map =
        std::vector<std::vector<Map_Cell>>(sizeY, std::vector<Map_Cell>(sizeX));
    ...
    for (uint16_t y = 0; y < sizeY; y++)
      for (uint16_t x = 0; x < sizeX; x++) {
        ...
        if (map[y][x].type == ANT_NEST_TYPE)
          ant_nests.push_back(
              Ant_Nest(sf::Vector2f((float)x * tileSize, (float)y * tileSize)));
      }
  ...
  void draw() {
    static sf::Color colors[] = {sf::Color::Transparent, sf::Color::Yellow,
                                 sf::Color(100, 100, 100), sf::Color::Green};
