...
struct Ant {
  ...
  void draw(Pheromone_Map &pheromone_map) {
    ...
    sf::Vector2f vertices[] = {
        {(startcellX - 1) * 2 * ant_sensing_distance,
         (startcellY - 1) * 2 * ant_sensing_distance},
        {(startcellX - 1) * 2 * ant_sensing_distance,
         endcellY * 2 * ant_sensing_distance},
        {endcellX * 2 * ant_sensing_distance,
         endcellY * 2 * ant_sensing_distance},
        {endcellX * 2 * ant_sensing_distance,
         (startcellY - 1) * 2 * ant_sensing_distance},
    };
    ...
    circle.setPosition(
        pos - sf::Vector2f(ant_sensing_distance, ant_sensing_distance));
    ...
    sf::Vector2f tail = pos + sf::Vector2f(cos(angle - TWO_PI) * antSize,
                                           sin(angle - TWO_PI) * antSize);
    sf::Vertex line[] = {pos, tail};
    window.draw(line, 2, sf::Lines);
  }

  void update(Pheromone_Map &pheromone_map) { behavior->update(pheromone_map); }
  ...
  bool is_near_pheromone(Pheromone *pheromone) {
    static const float ant_sensing_distance_squared =
        ant_sensing_distance * ant_sensing_distance;
    return magnitude_squared(pos - pheromone->getPos()) <
           ant_sensing_distance_squared;
  }
  ...
  bool can_emanate_pheromone() { return !((frameCount + offset) & timer); }
};
